...
|REDPEN_CONFIG_FILE |なし          |RedPen の設定ファイルです。指定する設定ファイルおよびリソース（JavaScript 拡張など）は RedPen のインストールディレクトリ（REDPEN_HOME）以下に保存してください。ファイルが REDPEN_HOME 以下に無い場合には、設定を読み込めません
|=======================================================================
