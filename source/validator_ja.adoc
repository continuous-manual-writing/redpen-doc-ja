...
この場合、ギャップを埋めるため 1.1.1 節の前に 1.1 節が存在するべきです。
...
日本語文に含まれる、曖昧な名詞接続のパターンを検出しエラーを出力します。検出される曖昧な名詞接続のパターンとは、名詞が前後に助詞「の」を伴なって接続される場合です。以下、パターンの例です。
...
日本語文において、計数表現のスタイルが食い違っている箇所を検出しエラーを出力します。
