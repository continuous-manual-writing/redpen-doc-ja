...
SentenceLength は入力文書内に存在する文（センテンス）の長さに関する規約が守られているかを検査します。
具体的には、文書内に存在する文が指定された最大文長より長い場合にエラーを出力します。
...
InvalidExpression は入力文が不正な表現（単語や句）を含むときにエラーを出力します。
...
どの言語にも対応しています。
ただし日本語や中国語のように単語の境界が曖昧な言語では InvalidExpression の利用をおすすすめします。
...
CommaNumber は文内のコンマの数が指定された最大数よりも多いときにエラーを出力します。
...
WordNumber はどの言語にも対応しています。
ただし、日本語やタイ語など、単語が半角スペースで分かれていない言語ではうまく動作しません。
...
辞書は二カラムのタブ区切り（TSV）フォーマットです。一列目には不正な表現、二列目に正しい表現を記述します。
以下辞書のサンプルとなります。
...
シンボルによっては代替のシンボルが存在します。たとえばクエスチョンマーク "? (0x003F)"
は代替のシンボル"？(0xFF1F)"がユニコードに登録されています。InvalidSymbol は入力文に不正なシンボル
が利用されているときにエラーを出力します。

文書内で使用するシンボルについての設定は設定ファイルの**symbols**ブロックで指定します。
...
シンボルによっては前もしくは後にスペースが必要です。たとえば、半角の左括弧 "(" の前に半角スペースを
入れる規約を矯正したい場合にはシンボルの左にスペースが必要という設定を追加します。
スペース前後の設定は設定ファイルの**symbols**ブロックで指定します。
...
KatakanaEndHyphen はカタカナ単語の語尾が規約（JIS Z8301、G.6.2.2 b、G.3.）にしたがっているかを検査します。
具体的には以下のルールが適用されます。
...
文中に対象となるカタカナ単語とスペリングが著しく類似する単語が存在するとエラーを出力します。

たとえば、入力ファイルに "インデックス" という単語が存在し、同時に "インデクス" が利用されている時に
KatakanaSpellCheck はエラーを出力します。
...
ParagraphNumber は一つの節内に存在してよいパラグラフの最大数を指定します。
...
ParagraphStartWith はパラグラフの開始部分が指定された規約にしたがっているかを検査します。
...
中国語や日本語など単語が空白で区切らない言語で文書執筆する際に、アルファベット単語の前後には空白を入れる規約を矯正するために使用します。
...
たとえば、以下の文では **良い** が二回使われているので、DoubledWord はエラーを出力します。
...
DoubledWord はトークナイザが対応していない言語で、かつ単語が分かれていない言語（中国語、タイ語など）に対応していません。
日本語は動作します。
...
たとえば入力文書が以下の文を含んでいる場合にエラーを出力します。
以下の文は、**言語**という単語をあやまって連続で使用しています。
...
DuplicatedSection は節の内容が他の節と類似しているときにエラーを出力します。
...
JapaneseStyle は**ですます調**と**である調**が混在して使われるとエラーを出力します。
...
FrequentSentenceStart は多くの文が同一の表現から開始されているときにエラーを出力します。
...
たとえば、もし ABC という省略形で書かれた単語が文中に存在した場合に、Axxx Bxx Cxxx という単語列が存在するかを検査します。
...
NumberFormatは英語の数値表現が一般的なフォーマットにしたがって記述されているかを検査します。
...
一文内での括弧の使用頻度が多い場合や、ネストされた括弧が存在するとエラーを出力します。
...
WeakExpression は文書内に"weak expression" と呼ばれる表現が現れるとエラーを出力します。
