...
機能拡張の作成には抽象クラス（Validator）を実装が必要です。
...
現在、二種類の prevalidte メソッドが提供されています。
...
PlainSentenceLengthValidator が設定ファイルに追加されると、RedPen は実装した vaildate メソッドを実行します。
...
作成したプラグインが設定ファイルに追加されると、RedPen は実装した vaildate メソッドを実行します。
