...
NOTE: 実装されたクラスは特定のパッケージに属す必要があります。利用できるパッケージは **cc.redpen.validator** 、 **cc.redpen.validator.sentence** 、 **cc.redpen.validator.section** の三つです。
...
コンストラクタで定義すると、RedPen は各 validator についてプロパティが必要かを判断できます。また IDE にプラグインとして組み込む場合においても、プラグインは必要なプロパティ定義を参照し GUI へ表示できます。
...
一旦設定されたプロパティは、+++getInt("プロパティ名")+++ のようにして参照できます。
...
上の例は Java 版と良く似ています。しかし型システムの違いによって Java 版における +++validate(Sentence sentence)+++ 関数が、JavaScript 版では +++validateSentence(sentence)+++ と呼ばれています。
